...
function _footerBlock_(theme, company) {
  ...
  return `<div style="position:relative;z-index:1;margin-top:34px;">
    ...
    <div style="background:${theme.primary};border-radius:10px;padding:14px 20px;text-align:center;">
      ...
      <p style="margin:3px 0 0;font-size:10px;color:rgba(255,255,255,.8);">Document gnr automatiquement par EntreFlow  ${company.name || ''}</p>
    </div>
  </div>`;
}
...
function loginWithGoogle_(p) {
  ...
  const token = uuid(); const now = new Date().toISOString();
  insertRow_(CONFIG.SHEETS.SESSIONS, { id: uuid(), token, user_email: email, debut_session: now, nb_actions: 1, derniere_action: 'login_google' });
