/* 
   ...
   Connexion employ par CODE + Portail employ & Login ultra pro (icnes SVG, temps rel)  Signature lectronique
   publique  Contrle de stock strict  Scurit renforce
   ...
   5. Dployer  Nouvelle version  Copier l'URL /exec
...
const CONFIG = {
  ...
  API_SECRET    : '',
  /* P1: pepper global pour le hashage des mots de passe  stock dans PropertiesService */
  PASSWORD_PEPPER : '',
  ...
  // et dans l'en-tte du portail employ. Laiss vide = un sigle "E" stylis
  // est utilis  la place.
...
const SHEET_SCHEMAS = {
  ...
  SessionsUtilisateurs : ['id','token','user_email','debut_session','nb_actions','derniere_action','last_seen','role','entity_id','ip','city','country','user_agent'],
...
function onOpen() {
  try {
    SpreadsheetApp.getUi().createMenu('EntreFlow')
      ...
      .addItem('Vrifier la configuration', 'checkConfiguration_')
      .addSeparator().addItem('Vider toutes les donnes', 'resetAllData')
      .addToUi();
  } catch (e) {}
}

/**
 * Contrle que CONFIG.PORTAL_URL a bien t remplac par la vraie URL
 * /exec du dploiement. Tant que ce n'est pas fait, tout lien envoy
 * par email (signature de devis, connexion employ) contient encore
 * "VOTRE_SCRIPT_ID"  un lien qui ne mne nulle part et que Google
 * Drive affiche comme "fichier introuvable". Ces deux fonctions
 * empchent ce genre de lien cass de partir silencieusement.
 */
...
function assertPortalUrlConfigured_() {
  if (!isPortalUrlConfigured_()) {
    throw new Error(
      "CONFIG.PORTAL_URL n'est pas configur. Va dans Dployer > Grer les dploiements, " +
      "copie l'URL qui se termine par /exec, et colle-la dans CONFIG.PORTAL_URL en haut du fichier. " +
      "Sans a, les liens envoys par email (signature, connexion employ) sont casss."
    );
  }
}
/** Menu EntreFlow > Vrifier la configuration  diagnostic rapide avant mise en prod */
function checkConfiguration_() {
  const ui = SpreadsheetApp.getUi();
  const lines = [];
  lines.push(isPortalUrlConfigured_()
    ? ' PORTAL_URL correctement configure.'
    : ' PORTAL_URL non configure  remplace VOTRE_SCRIPT_ID par ta vraie URL /exec.');
  const secretState = getApiSecret_() || CONFIG.API_SECRET || '';
  lines.push(secretState === 'CHANGE_ME_SECRET' || !secretState
    ? ' API_SECRET manquant ou par dfaut  configurez PropertiesService: API_SECRET'
    : ' API_SECRET personnalis.');
  lines.push(CONFIG.ENTREFLOW_LOGO_BASE64
    ? ' Logo EntreFlow configur.'
    : ' Logo EntreFlow non configur (un sigle "E" est utilis  la place  facultatif).');
  ui.alert('Diagnostic EntreFlow', lines.join('\n\n'), ui.ButtonSet.OK);
}
function setupSheet() { setupSheetApi(); SpreadsheetApp.getUi().alert('EntreFlow v14 initialis.'); }
...
function resetAllData() {
  const ui = SpreadsheetApp.getUi();
  if (ui.alert('Supprimer TOUTES les donnes ?', ui.ButtonSet.YES_NO) !== ui.Button.YES) return;
...
/* P1: hash mot de passe avec salt par utilisateur + pepper global  stock dans utilisateurs.salt */
function hashPassword(pw, salt) {
  const payload = String(pw || '') + '|' + String(salt || '') + '|' + String(getPasswordPepper_() || '');
...
/**
 * Code de connexion employ  court, facile  taper sur un tlphone,
 * sans caractres ambigus (pas de 0/O, 1/I/L). Unique par employ.
 * C'est CE code (envoy par email) qui identifie  la fois l'employ
 * ET son entreprise lors de la connexion  impossible de confondre
 * les employs de deux entreprises diffrentes, chaque code n'existe
...
function generateEmployeeCode_() {
  ...
  while (findRow_(CONFIG.SHEETS.EMPLOYEES, 'access_code', code)); // garantit l'unicit
  return code;
}
...
/* P1: utilitaire d'isolation multi-entreprise  vrifie qu'un enregistrement appartient bien  l'entreprise appelante */
function assertBelongsToCompany_(sheetName, id, companyId) {
  if (!id || !companyId) return true; // laiss passer les cas sans id (cration)
  ...
  if (col >= 0 && String(found.row[col] || '') !== String(companyId)) {
    throw new Error("Accs refus : cet enregistrement n'appartient pas  votre entreprise.");
  }
}
/* P1: chappement HTML pour prvenir XSS  utilis dans tous les templates HTML/email/PDF. */
...
function fmtDateTime(d) { const dt = new Date(d || Date.now()); return dt.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) + '  ' + dt.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }); }
...
/* 
     SCURIT  vrification d'accs pour toute action sensible
    */
/* P1: verifyAccess_ corrig le scope owner ne dpend plus d'un employ fantme ; il lit la session patron dans SessionsUtilisateurs puis valide l'utilisateur dans Utilisateurs. */
/* P2: suivi de presence "en ligne" pour le panneau super admin.
   On limite les ecritures a une fois par ~45s par session pour ne pas
   ralentir l'app avec une ecriture Sheet a chaque appel API. */
function bumpSessionLastSeen_(sess, meta) {
  try {
    const scol = k => sess.header.findIndex(h => h.trim() === k);
    const lastSeenRaw = sess.row[scol('last_seen')];
    const lastSeen = lastSeenRaw ? new Date(lastSeenRaw) : null;
    if (lastSeen && (Date.now() - lastSeen.getTime()) < 45 * 1000) return; // trop recent, on saute l'ecriture
    const rowId = sess.row[scol('id')];
    updateRow_(CONFIG.SHEETS.SESSIONS, rowId, {
      last_seen: new Date().toISOString(),
      role: meta.role || sess.row[scol('role')] || '',
      entity_id: meta.entity_id || sess.row[scol('entity_id')] || ''
    });
  } catch (e) { /* ne jamais bloquer une requete pour un souci de presence */ }
}
function bumpEmployeePresence_(emp) {
  try {
    const existing = findRow_(CONFIG.SHEETS.SESSIONS, 'token', emp.access_token);
    if (existing) {
      const scol = k => existing.header.findIndex(h => h.trim() === k);
      const lastSeenRaw = existing.row[scol('last_seen')];
      const lastSeen = lastSeenRaw ? new Date(lastSeenRaw) : null;
      if (lastSeen && (Date.now() - lastSeen.getTime()) < 45 * 1000) return;
      updateRow_(CONFIG.SHEETS.SESSIONS, existing.row[scol('id')], { last_seen: new Date().toISOString(), role: 'employee', entity_id: emp.id });
    } else {
      const now = new Date().toISOString();
      insertRow_(CONFIG.SHEETS.SESSIONS, { id: uuid(), token: emp.access_token, user_email: emp.email || '', debut_session: now, nb_actions: 1, derniere_action: 'employee_access', last_seen: now, role: 'employee', entity_id: emp.id, ip: '', city: '', country: '', user_agent: '' });
    }
  } catch (e) { /* idem, ne jamais bloquer la requete */ }
}
function verifyAccess_(data) {
  const token = String((data || {}).token || '').trim();
  if (!token) return { ok: false, error: 'Authentification requise.' };
  const apiSecret = getApiSecret_() || CONFIG.API_SECRET || '';
  if (apiSecret && token === apiSecret) return { ok: true, scope: 'admin' };

  const emp = findEmployeeByToken_({ token });
  if (emp && String(emp.statut) === 'actif') {
    const MAX_EMP_SESSION_HOURS = 72;
    const created = new Date(emp.created_at || Date.now());
    if ((Date.now() - created.getTime()) / 3600000 > MAX_EMP_SESSION_HOURS) {
      return { ok: false, error: 'Session employe expiree, veuillez vous reconnecter.' };
    }
    bumpEmployeePresence_(emp);
    return { ok: true, scope: 'employee', employee: emp };
  }

  // Sessions patron/propritaire cres par loginUser_
  const sess = findRow_(CONFIG.SHEETS.SESSIONS, 'token', token);
  if (sess) {
    const scol = k => sess.header.findIndex(h => h.trim() === k);
    const MAX_SESSION_HOURS = 72;
    const debutRaw = sess.row[scol('debut_session')];
    if (debutRaw) {
      const debut = new Date(debutRaw);
      if ((Date.now() - debut.getTime()) / 3600000 > MAX_SESSION_HOURS) {
    ...
    if (userFound) {
      ...
      if (String(userFound.row[ucol('statut')]).toLowerCase() !== 'suspendu') {
        ...
        bumpSessionLastSeen_(sess, { role: 'owner', entity_id: obj.id });
        return {
          ok: true,
          scope: 'owner',
  ...
  return { ok: false, error: 'Accs refus : identifiants invalides ou expirs.' };
}
function scopePayload_(data, access) {
  if (access.scope === 'employee') {
    ...
    data.company_id = access.company_id || '';
    data.companyId = access.company_id || '';
  }
  return data;
}
...
function registerUser(p) {
  ...
  if (findRow_(CONFIG.SHEETS.USERS, 'email', email)) throw new Error('Un compte existe dj avec cet email.');
  ...
  // P2: creation automatique d'une entreprise par defaut, pour ne plus bloquer l'utilisateur
  // avec un ecran de configuration obligatoire a l'inscription. Il pourra la personnaliser
  // plus tard depuis les Parametres (nom, logo, couleur, adresse...).
  const defaultCompanyName = p.companyName || (p.prenom ? `Entreprise de ${p.prenom}` : 'Mon Entreprise');
  const newCompany = createCompany_({ ownerId: id, email, name: defaultCompanyName });
  const token = uuid();
  insertRow_(CONFIG.SHEETS.SESSIONS, { id: uuid(), token, user_email: email, debut_session: now, nb_actions: 1, derniere_action: 'register', last_seen: now, role: p.role || 'admin', entity_id: id, ip: '', city: '', country: '', user_agent: '' });
  sendEmail_({ to: email, subject: `Bienvenue sur ${CONFIG.APP_NAME}  Votre espace est prt`, html: buildEmail_Welcome({ prenom: p.prenom, email, password: pw }) });
  logAudit_('CREATE_USER', 'Utilisateurs', id, email, `${p.prenom} ${p.nom}`);
  return { ok: true, user: { token, id, email, prenom: p.prenom || '', nom: p.nom || '', role: (p.role || 'admin'), company_id: (newCompany && newCompany.id) || '', start: now } };
}
/* P1: loginUser_ avec fallback legacy pour migration transparente des anciens hash sans sel + anti-brute-force */
function loginUser_(p) {
  const email = String((p || {}).email || '').trim().toLowerCase();
  const password = String((p || {}).password || '');
  if (!email || !password) return { ok: false, error: 'Email et mot de passe requis.' };
  const cacheKey = 'ef_login_fail_' + email;
  const cache = CacheService.getScriptCache();
  const fails = parseInt(cache.get(cacheKey) || '0', 10);
  if (fails >= 5) return { ok: false, error: 'Trop de tentatives. Reessayez dans 15 minutes.' };
  const found = findRow_(CONFIG.SHEETS.USERS, 'email', email);
  if (!found) return { ok: false, error: 'Email introuvable. Vrifiez l\'adresse ou inscrivez-vous.' };
  ...
  if (!ok) {
    cache.put(cacheKey, String(fails + 1), 15 * 60);
  ...
  insertRow_(CONFIG.SHEETS.SESSIONS, { id: uuid(), token, user_email: email, debut_session: now, nb_actions: 1, derniere_action: 'login', last_seen: now, role: found.row[col('role')] || 'owner', entity_id: found.row[col('id')], ip: '', city: '', country: '', user_agent: '' });
  updateRow_(CONFIG.SHEETS.USERS, found.row[col('id')], { derniere_connexion: now });
  logAudit_('LOGIN_OK', 'Utilisateurs', found.row[col('id')], email, 'Connexion russie' + (migrated ? ' (migre vers sel+pepper)' : ''));
...
function logoutUser_(p) { logAudit_('LOGOUT', 'Utilisateurs', '-', (p || {}).email || '', 'Dconnexion'); return { ok: true }; }
...
function forgotPassword(email) {
  ...
  const cache = CacheService.getScriptCache();
  if (cache.get(cacheKey)) throw new Error('Un email de rinitialisation a dj t envoy rcemment. Vrifiez votre bote ou attendez 5 minutes.');
  ...
  sendEmail_({ to: email, subject: `${CONFIG.APP_NAME}  Rinitialisation de mot de passe`, html: buildEmail_ResetPassword({ prenom: found.row[col('prenom')], tmp }) });
  cache.put(cacheKey, '1', 5 * 60);
  return { success: true };
}
/* P1: flux "mot de passe oubli" complet  3 tapes: demande/email, vrification code, reset. */
/* tape 1: demande de reset  envoie un code 6 chiffres par email au responsable */
function requestBossResetCode(p) {
  ...
  const cache = CacheService.getScriptCache();
  if (cache.get(cacheKey)) throw new Error('Un code a dj t envoy rcemment. Vrifiez votre bote ou attendez 5 minutes.');
  const found = findRow_(CONFIG.SHEETS.USERS, 'email', email);
  if (!found) throw new Error('Email introuvable.');
  const col = k => found.header.findIndex(h => h.trim() === k);
  ...
  const html = _emailWrap_('', 'Rinitialisation de mot de passe', `
    <h1 style="font-size:19px;font-weight:800;margin:0 0 8px;color:#0F172A;">Bonjour ${prenom || ''},</h1>
    <p style="color:#64748B;margin:0 0 16px;font-size:14px;">Voici votre code de rinitialisation. Il est valable 24 heures.</p>
    <table width="100%" style="background:#EFF6FF;border-radius:12px;border:1px solid #BFDBFE;"><tr><td style="padding:18px;text-align:center;">
      <p style="margin:0 0 8px;font-size:11px;color:#1D4ED8;text-transform:uppercase;letter-spacing:.08em;font-weight:800;">Votre code de rinitialisation</p>
      <p style="margin:0;font-size:30px;font-weight:900;letter-spacing:8px;color:#1D4ED8;font-family:Arial,sans-serif;">${code}</p>
    </td></tr></table>
    <p style="font-size:12px;color:#94A3B8;text-align:center;margin-top:14px;">Si vous n'avez pas demand cette rinitialisation, ignorez cet email.</p>
  `);
  sendEmail_({ to: email, subject: `${CONFIG.APP_NAME}  Code de rinitialisation`, html });
  cache.put(cacheKey, '1', 5 * 60);
  return { success: true, message: "Code envoy au responsable de l'entreprise." };
}

/* tape 2: vrification du code */
function verifyResetCode(p) {
  ...
  if (!match) throw new Error('Code invalide ou expir.');
  const token = Utilities.getUuid();
  updateRow_(CONFIG.SHEETS.RESET_CODES || 'ResetCodes', match.id, { used: 'true', used_at: now });
  return { ok: true, token };
}

/* tape 3: reset du mot de passe avec le token */
function resetBossPassword(p) {
  ...
  if (newPass.length < 6) throw new Error('Le mot de passe doit contenir au moins 6 caractres.');
  const resetCodes = sheetToObjects_(CONFIG.SHEETS.RESET_CODES || 'ResetCodes');
  const now = new Date().toISOString();
  const match = resetCodes.find(r => String(r.user_email || '').toLowerCase() === email && String(r.code || '') === code && String(r.used || 'false') === 'false' && new Date(r.expires_at || now) > new Date(now));
  if (!match) throw new Error('Code invalide ou expir.');
  ...
  updateRow_(CONFIG.SHEETS.RESET_CODES || 'ResetCodes', match.id, { used: 'true', used_at: now });
  logAudit_('PASSWORD_RESET', 'Utilisateurs', found.row[col('id')], email, 'Mot de passe rinitialis via code');
  return { success: true, message: 'Mot de passe rinitialis avec succs.' };
}
...
function createCompany_(p) {
  ...
  const obj = { id, owner_id: p.ownerId || '', name: p.name || '', phone: p.phone || '', email: p.email || '', address: p.address || '', logo_data: p.logo_data || '', logo_color: p.logo_color || '', tax_id: p.tax_id || '', rccm: p.rccm || '', currency: p.currency || 'XOF', tva_rate: p.tva_rate || 18, quote_prefix: p.quote_prefix || 'EF-', signature_data: p.signature_data || '', statut: 'actif', created_at: now, updated_at: now };
...
/* P1: updateCompany_  Empche la modification de l'entreprise d'un tiers ; utilise uniquement data.company_id inject par scopePayload_ */
function updateCompany_(p) {
  ...
  if (String(id) !== String(p.company_id || '')) return { ok: false, error: "Accs refus : modification impossible sur une autre entreprise." };
...
function deleteBranch_(p) {
  ...
  const br = rowToObject_(findRow_(CONFIG.SHEETS.BRANCHES, 'id', id));
  if (!br) return { ok: false, error: 'Agence introuvable' };
  if (String(br.company_id) !== String((p || {}).companyId || '')) return { ok: false, error: 'Accs refus' };
  ...
  if (emps.length && !mainBr) return { ok: false, error: 'Rattachez d\'abord les employs  une autre agence.' };
...
/* P1: updateProduct_  vrifie appartenance entreprise */
...
/* P1: deleteProduct_  vrifie appartenance entreprise */
...
/* P1: permanentDeleteProduct_  vrifie appartenance entreprise */
...
/* P3: decrementStock_  aprs dcrment, notifie le boss si le stock passe sous le seuil */
...
/**
 * Rapprovisionnement rapide  ajoute une quantit au stock existant
 * (au lieu d'craser la valeur comme updateStock_). Retourne l'ancienne
 * et la nouvelle quantit pour que le frontend affiche "Stock : 5  25"
 * dans sa petite fentre de rapprovisionnement.
 */
function restockProduct_(p) {
  ...
  if (!addQty || addQty <= 0) throw new Error('Quantit  ajouter invalide.');
  ...
  logAudit_('RESTOCK', 'Stocks', p.product_id, p.created_by || '', `+${addQty} (avant: ${before}, aprs: ${before + addQty})`);
...
function assertStockAvailable_(branchId, items) {
  ...
  (items || []).forEach(it => {
    ...
    if (requested > available) {
      if (available <= 0) throw new Error(`Stock puis pour "${name}"  aucune unit disponible.`);
      throw new Error(`Stock insuffisant pour "${name}" : ${available} disponible(s), ${requested} demand(s).`);
    }
  });
}
...
/* P1: updateClient_  vrifie appartenance entreprise */
...
/* P1: deleteClient_  vrifie appartenance entreprise */
...
function createEmployee_(p) {
  ...
  if (p.whatsapp) {
    ...
    obj.whatsapp_link = `https://wa.me/${wp}?text=${encodeURIComponent(`Bonjour ${p.fullName}, voici votre code d'accs  l'espace vendeur ${company.name || CONFIG.APP_NAME} : ${code}\nConnectez-vous ici : ${loginLink}`)}`;
  }
...
/**
 * Retrouve un employ  partir de son code de connexion  6 caractres.
 * Le code tant unique dans toute la feuille Employes, il identifie sans
 * ambigut l'employ ET l'entreprise pour laquelle il travaille  un
 * employ d'une entreprise ne peut jamais, avec son code, atterrir dans
 * les donnes d'une autre entreprise.
 */
...
/* P1: rate limiting login employ par code  max 5 tentatives par 15 min par IP/code */
function employeeLoginByCode_(p) {
  ...
  if (attempts >= 5) {
    ...
    return { ok: false, error: `Trop de tentatives. Ressayez dans ${remaining} minute(s).` };
  }
  ...
  if (!emp) {
    ...
    return { ok: false, error: 'Code invalide. Vrifiez le code reu par email.' };
  }
  if (String(emp.statut) !== 'actif') return { ok: false, error: 'Ce compte employ est inactif. Contactez votre responsable.' };
...
function getEmployeeDashboard_(p) {
  ...
  if (!emp) throw new Error('Accs refus : token invalide'); if (String(emp.statut) !== 'actif') throw new Error('Compte employ inactif');
  const company = getCompanyById_(emp.company_id) || {};
  ...
  /* P2: les clients sont filtrs par agence pour respecter la rgle d'isolation multi-entreprise */
...
function getEmployeeStats_(p) {
  ...
  if (!emp) throw new Error('Employ introuvable'); const empId = emp.id; const now = new Date(); const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
...
/**
 * Vue Boss uniquement (action admin)  pour un jour donn (aujourd'hui
 * par dfaut), combien chaque employ a vendu : nombre de ventes,
 * chiffre d'affaires, et le dtail des produits vendus par cet employ.
 * Plus le total gnral de l'entreprise pour la journe. Rserve au
 * scope 'admin'  un employ ne peut jamais appeler cette action pour
 * voir les chiffres de ses collgues (voir ADMIN_ONLY_ACTIONS).
 */
...
function saveSaleRecord_(p) {
  ...
  /* P3: vrification stock bas aprs vente et notification patron */
...
/* P1: updateSale_  vrifie que la vente appartient bien  l'entreprise de l'appelant */
/* P1/P4: updateSale_  vrification + dcrment stock sous verrou pour viter la survente concurrente */
...
/* P5: listSales_ limite par dfaut aux 90 derniers jours pour viter la dgradation progressive des temps de rponse ; passer all=true pour dsactiver la limite */
...
/* P1: cancelSale_  vrifie appartenance entreprise avant suppression */
...
/* 
     DEVIS  avec token de signature publique + thme visuel
    */
function createQuote_(p) {
  ...
  /* P3: notification patron sur nouveau devis employ */
  if (p.employee_id && p.company_id) {
    try { notifyBoss_(p.company_id, 'NEW_QUOTE', `Nouveau devis ${quoteNumber}`, `${getEmployeeName_(p.employee_id)} a cr un devis pour ${clientName || 'Client'} (${money(total)})`, { employee_id: p.employee_id, employee_name: getEmployeeName_(p.employee_id), montant: total }); } catch (e) {}
  }
...
/* P5: listQuotes_ limite par dfaut aux 90 derniers jours ; passer all=true pour dsactiver */
...
/* P1: deleteQuote_  vrifie appartenance entreprise */
...
/* P1: sendQuoteSignLink_  vrifie appartenance entreprise */
function sendQuoteSignLink_(p) {
  ...
  const { quote } = detail;
  if (!quote.client_email) throw new Error('Aucun email client renseign pour ce devis.');
  const company = getCompanyById_(quote.company_id) || {};
  const signLink = getPortalUrl_() + '/sign?sign=' + quote.id + '&t=' + quote.sign_token;
  sendEmail_({
    to: quote.client_email,
    subject: `${company.name || CONFIG.APP_NAME}  Devis N ${quote.quote_number}  valider`,
...
/* P3: logNotification_ enregistre dsormais company_id pour que les notifications soient visibles ct dashboard */
 ...
 * (cloche + compteur de non-lues), indpendamment des emails. Ce
 * n'est PAS une notification push de tlphone (a demanderait une
 * PWA + Firebase, hors-scope Apps Script)  c'est une notification
 * "en base", visible ds que le dashboard admin l'affiche ou se
 * rafrachit automatiquement.
 */
...
function confirmQuoteSignature_(p) {
  ...
  if (quote.statut === 'signe') return { ok: false, error: 'Ce devis a dj t sign.' };
  ...
  logAudit_('SIGN_QUOTE', 'Devis', quote.id, quote.client_email || '', `Sign par ${p.signerName}`);
  /* P3: notification patron sur devis sign */
  if (quote.employee_id && quote.company_id) {
    try { notifyBoss_(quote.company_id, 'QUOTE_SIGNED', `Devis ${quote.quote_number} sign`, `Le devis de ${quote.client_name || 'Client'} pour ${money(quote.total)} a t sign par ${p.signerName}`, { employee_id: quote.employee_id, employee_name: getEmployeeName_(quote.employee_id), montant: quote.total }); } catch (e) {}
  }
  // TODO P1: lien de signature permanent sans expiration  prvoir une date d'expiration plus tard si besoin.

  try {
    const updated = getQuoteDetail_({ id: quote.id }).quote;
    const company = getCompanyById_(quote.company_id) || {};
    const pdf = buildQuotePDFBlob_(updated, detail.items, company);
    if (updated.client_email) sendEmail_({ to: updated.client_email, subject: `${company.name || CONFIG.APP_NAME}  Devis N ${updated.quote_number} approuv`, html: buildEmail_QuoteApproved({ clientNom: updated.client_name, quoteNumber: updated.quote_number, total: updated.total, company }), attachments: [pdf] });
    if (company.email) sendEmail_({ to: company.email, subject: `Devis N ${updated.quote_number} approuv par ${updated.signed_by}`, html: buildEmail_QuoteApprovedNotifyOwner({ quoteNumber: updated.quote_number, clientNom: updated.client_name, signedBy: updated.signed_by, total: updated.total, company }), attachments: [pdf] });
  } catch (e) { console.error('post-signature email error', e); }

  return { ok: true };
}
/* P1: signQuote_  vrifie appartenance entreprise */
...
/* P1: generateQuotePDF_  vrifie appartenance entreprise */
...
function repondreAvis_(p) {
  ...
  if (found) { const col = k => found.header.findIndex(h => h.trim() === k); const toEmail = found.row[col('user_email')]; if (toEmail) sendEmail_({ to: toEmail, subject: `${CONFIG.APP_NAME}  Rponse  votre avis`, html: buildEmail_ReponseAvis({ reponse: p.reponse || '' }) }); }
  return r;
}
...
/* P1: contactAdmin_  chappement du message avant envoi email pour prvenir XSS */
...
/* 
     THMES VISUELS  devis/factures jamais identiques
    */
const DOC_THEMES = [
  ...
  { name: 'meraude',       primary: '#047857', primaryDark: '#065F46', soft: '#ECFDF5', border: '#A7F3D0' },
...
/* P2: petites conversions couleur pour deriver un theme complet a partir
   d'une seule couleur de logo (hex #RRGGBB). */
...
/** Construit un theme de document a partir de la couleur du logo de l'entreprise.
 *  Repli sur pickTheme_(seed) si aucune couleur de logo n'est enregistree. */
function themeFromCompany_(company, seed) {
  const hex = company && company.logo_color;
  const rgb = _hexToRgb_(hex);
  if (!rgb) return pickTheme_(seed);
  const hsl = _rgbToHsl_(rgb.r, rgb.g, rgb.b);
  const primary = _hslToHex_(hsl.h, Math.max(hsl.s, 35), Math.min(Math.max(hsl.l, 28), 45));
  const primaryDark = _hslToHex_(hsl.h, Math.max(hsl.s, 35), Math.max(hsl.l - 18, 12));
  const soft = _hslToHex_(hsl.h, Math.min(hsl.s, 60), 96);
  const border = _hslToHex_(hsl.h, Math.min(hsl.s, 55), 85);
  return { name: 'Couleur du logo', primary, primaryDark, soft, border };
}
...
/* 
     BLOCS RUTILISABLES DU NOUVEAU GABARIT DE DOCUMENT
    */
 ...
 * renseign (coller ici le contenu du fichier "logoEntre"), sinon un
 * sigle "E" stylis en secours.
 */
...
/** Logo de l'entreprise (rel si fourni, sinon badge  l'initiale) */
/* P2: logo de l'entreprise cliente en rond (demande produit), cadre en carre
...
/** Bande d'accent tout en haut du document (couleur du thme) */
...
/** En-tte : marque EntreFlow + tagline  gauche, gros titre du document + numro  droite */
/* P2: en-tete en marque blanche - affiche le logo/nom de l'entreprise cliente,
   plus aucune mention EntreFlow sur le document final. */
function _docHeader_(theme, docType, docNumber, extraBadgeHtml, company) {
  const co = company || {};
  return `<table width="100%" style="position:relative;z-index:1;margin-bottom:22px;">
    <tr>
      <td style="vertical-align:top;">
        ${_companyLogo_(co, 44)}
        <p style="font-size:15px;font-weight:800;color:#0F172A;margin:8px 0 0 0;font-family:Arial,sans-serif;">${escapeHtml_(co.name || '')}</p>
      </td>
...
/** Petite ligne d'info avec libell discret au-dessus de la valeur */
...
/** Bloc mta (dates, rfrence)  colonne de droite miroir du bloc metteur */
...
/** Bloc "Factur/Adress "  informations client relles uniquement */
...
/** Encart des totaux, avec la ligne finale mise en valeur (bandeau color) */
...
/** Pied de page color  coordonnes entreprise + mention EntreFlow (pas de QR, pas d'images tierces) */
...
/* 
     DEVIS  gabarit final
    */
/* P1: buildQuoteHTML_  toutes les variables injectes dans le HTML sont chappes */
function buildQuoteHTML_(quote, items, company) {
  const theme = themeFromCompany_(company, quote.theme_seed || quote.id);
...
<div style="max-width:760px;margin:0 auto;padding:38px 46px;position:relative;">
  ...
  ${_accentBar_(theme)}
  ${_docHeader_(theme, 'DEVIS', quote.quote_number, statusBadge, company)}
...
/* 
     FACTURE  gabarit final (simple, pure, tampon PAY)
    */
/* P1: buildInvoiceHTML_  donnes client chappes pour prvenir XSS */
function buildInvoiceHTML_(sale, items, company) {
  const theme = themeFromCompany_(company, sale.sale_number || sale.id);
  const currency = company.currency || 'F CFA';
  const money = v => fmtMoney(v, currency);
...
function loginWithGoogle_(p) {
  p = p || {};
  const idToken = String(p.id_token || '').trim();
  if (!idToken) return { ok: false, error: 'id_token Google requis.' };
  let email = String(p.email || '').trim().toLowerCase();
  const name = String(p.name || '').trim();
  try {
    const infoUrl = 'https://oauth2.googleapis.com/tokeninfo?id_token=' + encodeURIComponent(idToken);
    const infoResp = UrlFetchApp.fetch(infoUrl, { muteHttpExceptions: true });
    const infoJson = JSON.parse(infoResp.getContentText());
    email = String(infoJson.email || email).trim().toLowerCase();
    if (!email || infoJson.email_verified !== 'true') return { ok: false, error: 'Email Google non vrifi.' };
  } catch (e) {
    return { ok: false, error: 'Token Google invalide.' };
  }
  if (!email) return { ok: false, error: 'Email Google requis.' };

  let found = findRow_(CONFIG.SHEETS.USERS, 'email', email);
  let userId, prenom, nom, companyId;
  if (!found) {
    userId = uuid();
    prenom = name.split(' ')[0] || email.split('@')[0];
    nom = name.split(' ').slice(1).join(' ') || '';
    const now = new Date().toISOString();
    const userSalt = Utilities.getUuid();
    insertRow_(CONFIG.SHEETS.USERS, {
      id: userId, prenom, nom, email,
      password_hash: hashPassword(Utilities.getUuid(), userSalt),
      salt: userSalt, telephone: '', whatsapp: '', adresse: '',
      role: 'admin', company_id: '', statut: 'actif',
      derniere_connexion: now, created_at: now, updated_at: now
    });
    // P2: meme logique que l'inscription classique - entreprise par defaut auto-cree,
    // pas d'ecran de configuration bloquant.
    const newCompany = createCompany_({ ownerId: userId, email, name: prenom ? `Entreprise de ${prenom}` : 'Mon Entreprise' });
    companyId = (newCompany && newCompany.id) || '';
    logAudit_('CREATE_USER', 'Utilisateurs', userId, email, 'Inscription via Google');
  } else {
    const col = k => found.header.findIndex(h => h.trim() === k);
    userId = found.row[col('id')];
    prenom = found.row[col('prenom')] || '';
    nom = found.row[col('nom')] || '';
    companyId = found.row[col('company_id')] || '';
    updateRow_(CONFIG.SHEETS.USERS, userId, { derniere_connexion: new Date().toISOString() });
  }

  const token = uuid(); const now = new Date().toISOString();
  insertRow_(CONFIG.SHEETS.SESSIONS, { id: uuid(), token, user_email: email, debut_session: now, nb_actions: 1, derniere_action: 'login_google', last_seen: now, role: 'admin', entity_id: userId, ip: '', city: '', country: '', user_agent: '' });
  logAudit_('LOGIN_GOOGLE_OK', 'Utilisateurs', userId, email, 'Connexion via Google');
  return { ok: true, user: { token, id: userId, email, prenom, nom, role: 'admin', company_id: companyId || '', start: now } };
}
...
function sendEmail_(p) {
  try {
    const opts = { to: p.to, subject: p.subject, htmlBody: p.html, name: CONFIG.SENDER_NAME, replyTo: CONFIG.SUPPORT_EMAIL };
    if (p.attachments && p.attachments.length) opts.attachments = p.attachments;
    MailApp.sendEmail(opts);
  } catch (e) {
    logAudit_('EMAIL_ERROR', 'Emails', p.to || '', p.to || '', 'chec envoi email: ' + e.message);
    console.error('sendEmail_', e);
  }
}
...
/* P1: _emailWrap_ chappe systmatiquement les textes dynamiques pour prvenir XSS */
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;">
  ...
  <tr><td style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:0 0 18px 18px;padding:18px 30px;text-align:center;">
    <p style="margin:0;font-size:11px;color:#94A3B8;">${co.name ? co.name + '  ' : ''}Propuls par ${CONFIG.APP_NAME}  Africa Golden Digital, Cotonou, Bnin</p>
  </td></tr>
</table></td></tr></table></body></html>`;
}
function buildEmail_Welcome(p) {
  return _emailWrap_('', 'Votre espace professionnel est activ', `
    <h1 style="font-size:21px;font-weight:800;margin:0 0 8px;color:#0F172A;">Bienvenue, ${p.prenom || ''} !</h1>
    <p style="color:#64748B;margin:0 0 20px;font-size:14px;line-height:1.6;">Votre compte ${CONFIG.APP_NAME} est prt.</p>
    ...
    <p style="text-align:center;margin-top:24px;"><a href="${CONFIG.APP_URL}" style="background:#1D4ED8;color:#fff;padding:13px 30px;border-radius:999px;text-decoration:none;font-size:14px;font-weight:700;">Accder  mon espace</a></p>`);
}
function buildEmail_ResetPassword(p) {
  return _emailWrap_('', 'Rinitialisation de mot de passe', `
...
function buildEmail_EmployeeWelcome(p) {
  const co = p.company || {};
  return _emailWrap_('', `Bienvenue dans l'quipe`, `
    <h1 style="font-size:19px;font-weight:800;margin:0 0 8px;color:#0F172A;">Bonjour ${p.nom || ''} !</h1>
    <p style="color:#64748B;margin:0 0 16px;font-size:14px;">Vous avez t ajout(e)  l'quipe de <b>${co.name || "l'entreprise"}</b> en tant que <b>${p.poste || 'employ(e)'}</b>.</p>
    <table width="100%" style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;margin-bottom:16px;"><tr><td style="padding:18px 20px;text-align:center;">
      <p style="margin:0 0 8px;font-size:11px;color:#1D4ED8;text-transform:uppercase;letter-spacing:.08em;font-weight:800;">Votre code d'accs personnel</p>
    ...
    <table width="100%" style="background:#F8FAFC;border-radius:12px;border:1px solid #E2E8F0;margin-bottom:20px;"><tr><td style="padding:16px 18px;">
      <p style="margin:7px 0;font-size:13px;color:#0F172A;"> Stock disponible en temps rel</p>
      <p style="margin:7px 0;font-size:13px;color:#0F172A;"> Enregistrement de vos ventes</p>
      <p style="margin:7px 0;font-size:13px;color:#0F172A;"> Cration de devis</p>
...
function buildEmail_SaleConfirmation(p) {
  ...
  return _emailWrap_('', 'Votre facture', `
    ...
    <p style="color:#64748B;margin:0 0 18px;font-size:13px;">Facture N ${p.saleNumber}  le document PDF est joint  cet email.</p>
...
function buildEmail_QuoteSignLink(p) {
  const co = p.company || {};
  return _emailWrap_('', 'Un devis attend votre validation', `
    <h1 style="font-size:19px;font-weight:800;margin:0 0 8px;color:#0F172A;">Bonjour ${p.clientNom},</h1>
    <p style="color:#64748B;margin:0 0 16px;font-size:14px;line-height:1.6;">${co.name || CONFIG.APP_NAME} vous a envoy le devis N <b>${p.quoteNumber}</b> d'un montant de <b>${fmtMoney(p.total, co.currency)}</b>, valable ${p.validiteJours} jours.</p>
    <p style="color:#64748B;margin:0 0 22px;font-size:14px;">Cliquez sur le bouton ci-dessous pour consulter le devis et le signer lectroniquement en ligne.</p>
    ...
    <p style="font-size:11px;color:#94A3B8;text-align:center;margin-top:16px;">Ce lien est personnel et scuris.</p>`, co);
}
function buildEmail_QuoteApproved(p) {
  const co = p.company || {};
  return _emailWrap_('', 'Devis approuv', `
    <h1 style="font-size:19px;font-weight:800;margin:0 0 8px;color:#0F172A;">Merci, ${p.clientNom} !</h1>
    <p style="color:#64748B;margin:0 0 18px;font-size:14px;">Votre signature du devis N ${p.quoteNumber} a bien t enregistre. Une copie PDF signe est jointe  cet email.</p>
    <p style="font-size:21px;font-weight:900;color:#1D4ED8;">Total : ${fmtMoney(p.total, co.currency)}</p>`, co);
}
function buildEmail_QuoteApprovedNotifyOwner(p) {
  const co = p.company || {};
  return _emailWrap_('', 'Un devis vient d\'tre approuv', `
    <h1 style="font-size:19px;font-weight:800;margin:0 0 8px;color:#0F172A;">Bonne nouvelle !</h1>
    <p style="color:#64748B;margin:0 0 8px;font-size:14px;">Le devis N <b>${p.quoteNumber}</b> a t sign par <b>${p.signedBy}</b> (${p.clientNom}).</p>
    <p style="font-size:21px;font-weight:900;color:#1D4ED8;">Total : ${fmtMoney(p.total, co.currency)}</p>`, co);
}
function buildEmail_ReponseAvis(p) {
  return _emailWrap_('', 'Rponse  votre avis', `<h1 style="font-size:18px;font-weight:800;margin:0 0 12px;color:#0F172A;">Merci pour votre retour !</h1><div style="background:#F8FAFC;border-radius:10px;padding:16px;font-size:14px;color:#0F172A;">${p.reponse}</div>`);
}
...
/* 
     PAGE PUBLIQUE DE SIGNATURE  servie via doGet(?sign=&t=)
    */
/* P1: buildSignPageHTML_  chappement des donnes du devis dans la page de signature publique */
...
<div class="wrap">
  <div class="card">
    ...
    <div class="badge ${alreadySigned ? 'badge-signed' : 'badge-pending'}">${alreadySigned ? ' Dj approuv' : 'En attente de votre validation'}</div>
    <p style="color:#64748B;font-size:13px;margin-top:14px;">mis par <b>${company.name || CONFIG.APP_NAME}</b> le ${fmtDate(quote.created_at)}</p>
    <table style="margin-top:18px;"><thead><tr><th>Article</th><th style="text-align:center;">Qt</th><th style="text-align:right;">Total</th></tr></thead><tbody>${rows}</tbody></table>
  ...
  <div class="card" style="text-align:center;">
    <p style="font-size:15px;color:#16A34A;font-weight:700;"> Ce devis a dj t sign par ${quote.signed_by} le ${fmtDate(quote.signed_at)}.</p>
  </div>` : `
  <div class="card">
    ...
    <div class="fg"><label>Votre nom complet *</label><input type="text" id="signerName" placeholder="Nom et prnom"></div>
    <div class="fg">
  ...
  <p class="footer">Signature scurise propulse par EntreFlow</p>
</div>
...
async function submitSignature(){
  ...
  try {
    ...
    okEl.textContent = 'Devis sign avec succs ! Une copie vous a t envoye par email.';
...
/* 
     PORTAIL EMPLOY  HTML servi via doGet(?employeeToken=)
    */
/* 
   EntreFlow  Portail Employ v2 (PRO)
   
    COLLER  LA PLACE de la fonction buildEmployeePortalHTML_ existante
   dans le fichier .gs (section   PORTAIL EMPLOY). Aucune autre
   fonction du backend n'a besoin de changer  cette page utilise les
   mmes actions API (getEmployeeDashboard, createSale, createQuote,
   createClient) dj en place.

   Ce qui change :
   - Design pur, sans aucun emoji : uniquement des icnes SVG fines
   - Navigation en pastilles en haut sur desktop/tablette, et barre
     de navigation fixe en bas de l'cran sur mobile ( 640px)
   - Indicateur "En direct" + rafrachissement automatique en tche de
     fond (toutes les 20 secondes) : si le chef rapprovisionne le
     stock ou qu'un collgue vend un produit, a se reflte sans avoir
      recharger la page
   - Stock en lecture seule ct employ (l'ajout de stock reste une
     action du dashboard boss  cohrent avec la sparation demande)
    */
/**
 * Icnes SVG gnres CT SERVEUR (Apps Script)  utilises dans le
 * gabarit HTML au moment de sa construction (nav, KPI, boutons de
 * fermeture des fentres). Un second jeu identique existe ct
 * navigateur, dans le <script> de la page, pour les lments ajouts
 * dynamiquement aprs chargement (toasts, lignes de vente/devis) 
 * les deux ne s'excutent jamais dans le mme contexte, donc aucun
 * conflit de nom malgr le mme nom de fonction `_i`.
 */
function _i(name) {
  const paths = {
...
function buildEmployeePortalHTML_(emp, dashData) {
  ...
  const quoteRows = (dashData.quotes || []).map(q => `<tr><td style="color:var(--blue-l);font-weight:500">${q.quote_number || ''}</td><td>${q.client_name || ''}</td><td style="font-weight:600">${money(q.total)}</td><td><span class="badge ${q.statut === 'signe' ? 'b-ok' : 'b-info'}">${q.statut === 'signe' ? 'Approuv' : 'En attente'}</span></td></tr>`).join('');
...
<div class="main">
  ...
  <div class="page" id="tab-products">
    <div class="card">
      ...
      <div class="tbl-wrap"><table><thead><tr><th>Produit</th><th>Unit</th><th>Prix</th><th>Stock</th></tr></thead><tbody id="prodBody">${productRows || '<tr><td colspan="4"><div class="empty-state"><strong>Aucun produit</strong></div></td></tr>'}</tbody></table></div>
    </div>
  </div>
  ...
  <div class="page" id="tab-clients">
    <div class="card">
      ...
      <div class="tbl-wrap"><table><thead><tr><th>Nom</th><th>Tlphone</th><th>Ville</th></tr></thead><tbody id="clientBody">${clientRows || '<tr><td colspan="3"><div class="empty-state"><strong>Aucun client</strong></div></td></tr>'}</tbody></table></div>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="saleModal"><div class="modal-box">
  ...
  <div class="row3">
    <div class="fg"><label>Paiement</label><select id="sl-method"><option value="cash">Espces</option><option value="mobile_money_wave">Wave</option><option value="mobile_money_mtn">MTN Money</option><option value="mobile_money_moov">Moov Money</option><option value="card">Carte</option><option value="bank_transfer">Virement</option></select></div>
    <div class="fg"><label>Montant reu</label><input id="sl-paid" type="number" min="0" placeholder="0" oninput="calcChange()"></div>
    <div class="fg"><label>Monnaie  rendre</label><input id="sl-change" type="text" value="0 F CFA" disabled style="opacity:.7;"></div>
  </div>
  <div class="actions">
...
<div class="modal-bg" id="quoteModal"><div class="modal-box">
  ...
  <div id="qt-new-client">
    <div class="row2">
      ...
      <div class="fg"><label>Tlphone (WhatsApp)</label><input type="tel" id="qt-cp" placeholder="+229 XX XX XX XX"></div>
    </div>
    <div class="row2">
      <div class="fg"><label>Email client</label><input type="email" id="qt-ce" placeholder="client@email.com"></div>
    </div>
  </div>
  <div class="fg"><label>Validit (jours)</label><input type="number" id="qt-validite" value="7" min="1"></div>
...
<div class="modal-bg" id="clientModal"><div class="modal-box">
  ...
  <div class="row2"><div class="fg"><label>Tlphone</label><input type="tel" id="nc-phone"></div><div class="fg"><label>Email</label><input type="email" id="nc-email"></div></div>
...
<script>
/* P2: chappement ct client des valeurs injectes dans le DOM pour prvenir XSS */
function esc(str){
  if(!str) return '';
...
/* P2: dconnexion employ  redirige vers la page de login par code */
...
let saleLineIdx = 0;
/* P2: ajout ligne vente  SVG pour suppression, noms chapps, validation stock */
function addSaleRow(){
...
/* P2: calcSaleTotal vrifie le stock et dsactive le bouton Enregistrer si ncessaire */
function calcSaleTotal(){
  ...
  // vrifie aussi les lignes de devis si ouvertes
  let qOver = false;
...
async function saveSale(){
  ...
  try {
    ...
    toast('Vente enregistre  ' + money(res.total) + (document.getElementById('sl-client-email').value.trim() ? '  Facture envoye.' : ''));
...
let quoteLineIdx = 0;
/* P2: ajout ligne devis  SVG suppression, noms chapps, validation stock */
...
async function saveQuote(){
  ...
  try {
    ...
    toast(res.signLinkSent ? 'Devis cr  lien envoy' : 'Devis cr  ' + res.quoteNumber);
...
/* P2: rafrachissement automatique du dashboard employ toutes les 25s */
...
<div class="card">
  ...
  <p class="sub">Entrez le code de connexion  6 caractres reu par email pour accder  votre tableau de bord.</p>
  ...
  <div class="hint">${_i('mail')}<p>Le code vous a t envoy par email lors de votre ajout  l'quipe. Vrifiez vos spams si vous ne le trouvez pas, ou contactez votre responsable.</p></div>

  <p class="footer">Propuls par <b>EntreFlow</b></p>
</div>
<script>
const API_URL = '${CONFIG.PORTAL_URL}';
...
async function tryLogin(){
  ...
  if (code.length < 6){ showErr('Le code doit contenir 6 caractres.'); return; }
...
function servePublicSignPage_(quoteId, token) {
  try {
    ...
    if (!detail || String(detail.quote.sign_token) !== String(token)) {
      return HtmlService.createHtmlOutput('<html><body style="font-family:Arial;padding:40px;text-align:center;background:#F1F5F9;"><h2>Lien invalide</h2><p>Ce lien de signature est invalide ou a expir.</p></body></html>').setTitle('Lien invalide');
    }
...
function serveEmployeePortal_(token) {
  try {
    const emp = findEmployeeByToken_({ token });
    if (!emp) return HtmlService.createHtmlOutput('<html><body style="font-family:Arial;padding:40px;text-align:center;background:#040810;color:#EEF2FF;"><h2>Accs refus</h2><p>Lien invalide ou expir.</p></body></html>').setTitle('EntreFlow  Accs refus');
...
function getPasswordPepper_() {
  const stored = PropertiesService.getScriptProperties().getProperty('PASSWORD_PEPPER');
  if (stored) return String(stored).trim();
  return '';
}
function getSuperAdminSecret_() {
  const stored = PropertiesService.getScriptProperties().getProperty('ENTREFLOW_SUPER_ADMIN_SECRET');
  if (stored) return String(stored).trim();
  throw new Error('Secret super admin non configure.');
}
...
function superAdminOverview_(token) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Accs refus.' };
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
...
function superAdminListCompanies_(token) {
  ...
  try {
    ...
    const employeesSh = ss.getSheetByName(CONFIG.SHEETS.EMPLOYEES);
    if (!companiesSh) return { ok: false, error: 'Feuille entreprises introuvable.' };
...
function superAdminListEmployees_(token) {
  ...
  try {
    ...
    const employeesSh = ss.getSheetByName(CONFIG.SHEETS.EMPLOYEES);
    const companiesSh = ss.getSheetByName(CONFIG.SHEETS.COMPANIES);
    if (!employeesSh) return { ok: false, error: 'Feuille employs introuvable.' };
...
function superAdminListOnline_(token) {
  ...
  try {
    ...
    const now = Date.now();
    const sessions = sheetToObjects_(CONFIG.SHEETS.SESSIONS);
    const employees = sheetToObjects_(CONFIG.SHEETS.EMPLOYEES);
    const users = sheetToObjects_(CONFIG.SHEETS.USERS);
    const companies = sheetToObjects_(CONFIG.SHEETS.COMPANIES);
    const empMap = {}; employees.forEach(e => empMap[String(e.id)] = e);
    const userMap = {}; users.forEach(u => userMap[String(u.id)] = u);
    const compMap = {}; companies.forEach(c => compMap[String(c.id)] = c.name);
    const list = [];
    sessions.forEach(s => {
      if (!s.last_seen) return;
      const last = new Date(s.last_seen);
      if (isNaN(last.getTime()) || (now - last.getTime()) > TWO_MIN_MS) return;
      let label = s.user_email || '', role = s.role || '', companyName = '';
      if (role === 'employee') {
        const emp = empMap[String(s.entity_id)];
        if (emp) { label = emp.full_name || label; companyName = compMap[String(emp.company_id)] || ''; }
      } else {
        const u = userMap[String(s.entity_id)];
        if (u) { label = (u.prenom || '') + ' ' + (u.nom || ''); companyName = compMap[String(u.company_id)] || ''; }
      }
      list.push({
        label: String(label).trim(), role, company_name: companyName,
        city: s.city || '', country: s.country || '', ip: s.ip || '',
        user_agent_short: String(s.user_agent || '').slice(0, 40),
        last_seen: last
      });
    });
    ...
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][0]) === companyId) {
        ...
        if (String(data[i][13]).toLowerCase() !== String(newStatut).toLowerCase()) {
          ...
          logAudit_('superAdminSuspendCompany', 'Boutiques', companyId, 'superadmin', 'Statut entreprise chang en ' + newStatut + ' par Super Admin');
        }
        break;
      }
    }
    if (suspend && employeesSh) {
      ...
      for (let i = 1; i < edata.length; i++) {
        if (String(edata[i][1]) === companyId && String(edata[i][11]).toLowerCase() === 'actif') {
          ...
          logAudit_('superAdminSuspendCompany', 'Employes', String(edata[i][0]), 'superadmin', 'Employ suspendu automatiquement avec l\'entreprise.');
        }
      }
    }
    return { ok: true };
...
function superAdminDeleteCompany_(token, payload) {
  ...
  try {
    ...
    logAudit_('superAdminDeleteCompany', 'Boutiques', companyId, 'superadmin', 'Entreprise supprime par Super Admin.');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
function superAdminSuspendEmployee_(token, payload) {
  ...
  try {
    ...
    const suspend = !!(payload && payload.suspend);
    if (!employeeId) return { ok: false, error: 'employeeId requis.' };
    const data = employeesSh.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][0]) === employeeId) {
        ...
        if (String(data[i][11]).toLowerCase() !== String(newStatut).toLowerCase()) {
          ...
          logAudit_('superAdminSuspendEmployee', 'Employes', employeeId, 'superadmin', 'Statut employ chang en ' + newStatut);
        }
        break;
      }
    }
...
function superAdminDeleteEmployee_(token, payload) {
  ...
  try {
    ...
    logAudit_('superAdminDeleteEmployee', 'Employes', employeeId, 'superadmin', 'Employ supprim par Super Admin.');
    return { ok: true };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
function superAdminListQuotes_(token) {
...
function superAdminListSales_(token) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
...
function superAdminListProducts_(token) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const products = sheetToObjects_(CONFIG.SHEETS.PRODUCTS);
...
function superAdminListTrash_(token) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const items = [];
...
function superAdminDeleteQuote_(token, payload) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const id = String(payload && payload.id || '').trim();
    if (!id) return { ok: false, error: 'id requis.' };
    const sh = ss.getSheetByName(CONFIG.SHEETS.QUOTES);
    if (!sh) return { ok: false, error: 'Feuille Devis introuvable.' };
    const rows = sh.getDataRange().getValues();
    const header = rows[0].map(String);
    const col = k => header.findIndex(h => h.trim() === k);
    const idCol = col('id');
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][idCol]) === id) {
        if (col('statut') >= 0) sh.getRange(i + 1, col('statut') + 1).setValue('supprime');
        logAudit_('superAdminDeleteQuote', 'Devis', id, 'superadmin', 'Devis supprime par Super Admin.');
        return { ok: true };
      }
    }
...
function superAdminCancelSale_(token, payload) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const id = String(payload && payload.id || '').trim();
    if (!id) return { ok: false, error: 'id requis.' };
    const sh = ss.getSheetByName(CONFIG.SHEETS.SALES);
    ...
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][idCol]) === id) {
        if (col('statut') >= 0) sh.getRange(i + 1, col('statut') + 1).setValue('cancelled');
        logAudit_('superAdminCancelSale', 'Ventes', id, 'superadmin', 'Vente annulee par Super Admin.');
        return { ok: true };
      }
    }
...
function superAdminDeleteClient_(token, payload) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try { return updateRow_(CONFIG.SHEETS.CLIENTS, String(payload && payload.id || '').trim(), { statut: 'inactif' }); }
  catch (e) { return { ok: false, error: e.message }; }
}
function superAdminDeleteProduct_(token, payload) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
  try { return updateRow_(CONFIG.SHEETS.PRODUCTS, String(payload && payload.id || '').trim(), { statut: 'inactif' }); }
  catch (e) { return { ok: false, error: e.message }; }
}
function superAdminRestoreTrash_(token, payload) {
  if (!requireSuperAdmin_(token)) return { ok: false, error: 'Acces refuse.' };
...
/** Actions strictement rserves au boss (scope 'admin')  jamais un employ */
/* P1: createCompany retir de la liste publique  rserv  l'exploitant du SaaS. */
...
function handleRequest_(e) {
  try {
    ...
    if (SCOPED_ACTIONS.has(action) || ADMIN_ONLY_ACTIONS.has(action)) {
      ...
      if (ADMIN_ONLY_ACTIONS.has(action) && access.scope !== 'admin' && access.scope !== 'owner') return jsonResp_({ ok: false, error: "Accs rserv au responsable de l'entreprise." });
    ...
    let result;
    switch (action) {
      case 'registerUser':            return jsonResp_(registerUser(data));
      case 'loginWithGoogle':         return jsonResp_(loginWithGoogle_(data));
      case 'requestBossResetCode':    return jsonResp_(requestBossResetCode(data));
      case 'verifyResetCode':         return jsonResp_(verifyResetCode(data));
      case 'resetBossPassword':       return jsonResp_(resetBossPassword(data));
      ...
      case 'superAdminOverview':      return jsonResp_(superAdminOverview_(data.token, data));
      case 'superAdminLogin':        return jsonResp_(superAdminLogin_(data.payload || data));
      case 'superAdminListCompanies':return jsonResp_(superAdminListCompanies_(data.token));
      case 'superAdminListEmployees':return jsonResp_(superAdminListEmployees_(data.token));
      case 'superAdminListOnline':    return jsonResp_(superAdminListOnline_(data.token));
      case 'superAdminActivityFeed':  return jsonResp_(superAdminActivityFeed_(data.token, data));
      case 'superAdminSuspendCompany':return jsonResp_(superAdminSuspendCompany_(data.token, data));
      case 'superAdminDeleteCompany': return jsonResp_(superAdminDeleteCompany_(data.token, data));
      case 'superAdminSuspendEmployee': return jsonResp_(superAdminSuspendEmployee_(data.token, data));
      case 'superAdminDeleteEmployee': return jsonResp_(superAdminDeleteEmployee_(data.token, data));
      case 'superAdminListQuotes':      return jsonResp_(superAdminListQuotes_(data.token));
      case 'superAdminListSales':       return jsonResp_(superAdminListSales_(data.token));
      case 'superAdminListProducts':    return jsonResp_(superAdminListProducts_(data.token));
      case 'superAdminListClients':     return jsonResp_(superAdminListClients_(data.token));
      case 'superAdminListTrash':       return jsonResp_(superAdminListTrash_(data.token));
      case 'superAdminDeleteQuote':     return jsonResp_(superAdminDeleteQuote_(data.token, data));
      case 'superAdminCancelSale':      return jsonResp_(superAdminCancelSale_(data.token, data));
      case 'superAdminDeleteClient':    return jsonResp_(superAdminDeleteClient_(data.token, data));
      case 'superAdminDeleteProduct':   return jsonResp_(superAdminDeleteProduct_(data.token, data));
      case 'superAdminRestoreTrash':    return jsonResp_(superAdminRestoreTrash_(data.token, data));
      case 'superAdminCreateQuote':     return jsonResp_(superAdminCreateQuote_(data.token, data));
      case 'superAdminGenerateQuotePDF': return jsonResp_(superAdminGenerateQuotePDF_(data.token, data));
